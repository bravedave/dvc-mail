...
abstract class query {
	...
	static public function MoveItem( $request, $creds = NULL ) {
		if ( $ews = client::instanceForDelete( $creds)) {
			...
			if ( isset( $response->ResponseMessages->MoveItemResponseMessage)) {
				...
				$ResponseMessage = $response->ResponseMessages->MoveItemResponseMessage[0];
				// sys::logger( print_r( $response->ResponseMessages->MoveItemResponseMessage[0], true));
				if ( 'Success' == $ResponseMessage->ResponseClass) {
					// sys::logger( sprintf('%s : %s', $ResponseMessage->ResponseClass, __METHOD__));
					return true;

				}

				// sys::logger( sprintf('%s : %s', $ResponseMessage, __METHOD__));

			}
			else {
				// sys::logger( print_r( $response->ResponseMessages->MoveItemResponseMessage[0], true));
				sys::logger( sprintf('responseClass not set : %s', __METHOD__));
