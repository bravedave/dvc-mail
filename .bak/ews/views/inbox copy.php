...
$(document).on( 'mail-folderlist', function( e) {
	...
	_brayworth_.post({
		...
		if ( 'ack' == d.response) {
			...
			$.each( d.folders, function( i, fldr) {
				// console.log( fldr);

				$('<li class="pt-1 py-md-0 py-3 pointer" />')
				.append( $('<div class="text-truncate" />').html( fldr.name))
				...
				.on( 'drop', function( e) {	/* drag drop move messages */
					...
					$('#' + data).trigger('execute-action', _data.folder);
					return;
					...
					if ( src.length) {
						...
						let uid = _data.message.messageid;
						let folder = _data.folder;
						let action = 'message-move';
						let target = $( e.originalEvent.target).data('folder');
						// console.log(e);

						console.log( uid, folder, action, target);
						// todo: execute the move
						// ControlActionExecuteOnUID( uid, folder, action, target);
...
$(document).on( 'mail-messages', function( e, folder) {
	...
	_brayworth_.post({
		...
		if ( 'ack' == d.response) {
			// console.log( d);
			$('#<?= $uidMsgs ?>').html('<div class="row bg-light text-muted"><div class="col"><h6 class="m-0">Messages</h6></div></div>');
			...
			$.each( d.messages, function( i, msg) {
				...
				// console.log( msg);
				let from = $('<div class="col text-truncate" />').html( msg.from);
				if ( 'no' == msg.seen) from.addClass('font-weight-bold');

				let received = $('<div class="col-3 pl-0 text-right text-truncate small" />');
				let subject = $('<div class="col-9 text-truncate" />').html( msg.subject).attr( 'title', msg.subject);
				...
				.on( 'execute-action', function( e, action) {
					let _me = $(this);
					let _data = _me.data();
...
/*
	...
	if ( !!targetFolder)
		ajaxData.append('target', targetFolder);
	...
	.always( function(data) {
		$('[data-role="loading"]').addClass('hidden');

	})
	;

	var clickNextRow = false;
	$('tr[data-role="message"]').each( function( i, el) {
		var r = $(el);
		if ( clickNextRow) {
			r.trigger('click');
			return ( false);
	...
	var container = $('#inbox-preview-panel');
	if ( /(string|number)/.test( typeof container.data('uid'))) {
		if ( uid == container.data('uid'))
			$('#inbox-preview-panel, #inbox-general-control-container').html('');
