...
class inbox
{
  ...
  public function SaveToFile($message, $msgStore)
  {
    ...
    if (!is_dir($msgStore)) mkdir($msgStore);
    ...
    $j = [
      ...
      'attachments' => array_map(fn ($a) => (object)[
        'Name' => $a->Name ?? 'attachment' . $fileName++,
        'ContentId' => $a->ContentId ?? 'no content id'
      ], $message->attachments)
    ];
    ...
    if (file_exists($file)) {
      ...
      if ($debug) file_put_contents($file, json_encode($j, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT));
      if ($debug) logger::debug(sprintf('<msg exists : %s : replaced> %s', $file, __METHOD__));
    } else {
