...
class MimeMessage {
  ...
  public function getHeaders() {
    ...
    return (string)$this->msg->extract_headers(MAILPARSE_EXTRACT_RETURN);
  }
  ...
  public function display_part_info($caption) {
    ...
    if ($n == 0) {
       ...
       * acts just as it does in extract_headers method.
       */
      $body = (string)$this->msg->extract_body(MAILPARSE_EXTRACT_RETURN);
  ...
  public function SaveToFile(PHPMailer $message, $msgStore, array $files = []) {
    ...
    if (!is_writable($msgStore)) throw new RuntimeException('path not writeable ' . $msgStore);
    ...
    if (!is_writable($attachmentPath)) throw new RuntimeException('path not writeable ' . $attachmentPath);
